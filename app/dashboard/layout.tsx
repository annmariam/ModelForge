                    ...
                    <div className="flex flex-col h-[calc(100vh-180px)] justify-between">
                        <nav>
                            {navItems[role]?.map((item) => (
                                <Link key={item.name} href={item.href} className={`flex items-center px-4 py-2 text-gray-700 hover:bg-gray-200 dark:text-gray-300 dark:hover:bg-gray-700 ${pathName === item.href ? "bg-gray-200 dark:bg-gray-700" : "text-red-400" }`}> {item.name} </Link>
                            ))}
                        </nav>
