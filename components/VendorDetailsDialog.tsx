    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl">
        ...
        {loading ? (
          ...
          <VendorDetails data={vendorData} />
        ) : (
          <p>No data available for this vendor.</p>
        )}
      </DialogContent>
    </Dialog>
  );
              ...
              {data.printerDetails.map((printer) => (
                <Card key={printer.id}>
                  <CardContent className="pt-4">
                    <InfoItem label="Printer Name" value={printer.name} />
                    <InfoItem label="Status" value={printer.status} />
                  </CardContent>
                </Card>
              ))}
            </div>
